...
export const updateCounters = (theToken) => {
  return axios
    .post(global.config.routerPath + '/donors/updatecounters2', {
      token: theToken,
...
export const updateKey = (title,oldKey, theToken) => {
  return axios
    .post(global.config.routerPath + '/donors/updatekey2', {
      title: title,
      oldKey: oldKey,
      token:theToken,
    ...
    .catch(err => {
      console.log("ClientSide Error @ DonorFunctions > update donor key" + err)
      return false
    })
}
...
export const deleteGroupFunct = (donorKey, theToken) => {
  return axios
    .post(global.config.routerPath + '/donors/deletegroup2', {
      donorKey: donorKey,
      token:theToken,
...
export const downLoadCSV = theToken => {
  console.log('in download csv')
    return axios
      .post(global.config.routerPath + '/donors/downloadcsv2',{
        token:theToken,
...
export const upload = () => {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            url: global.config.routerPath + '/donors/uploadcsv2',
            data: data,
            cache: false,
            contentType: false,
            processData: false,
...
export const deleteSQL = (id,modelName, theToken) => {
  ...
  return axios
    .post(global.config.routerPath + '/donors/deletesql2', jo )
    .then(res => {
      return res.data
    })
    .catch(err => {
...
export const updateDonorsSQL = (ids,newVals,fields,theToken) => {
  ...
  return axios
    .post(global.config.routerPath + '/donors/updatesqldonors2', jo)
    .then(res => {
      return res.data
    })
    .catch(err => {
...
export const getDonors = (rest, theToken) => {
    return axios
      .post(global.config.routerPath + '/donors/getdonors2',{
          rest:rest, 
          token: theToken,
...
export const getDonorCats = theToken => {
    return axios
      .post(global.config.routerPath + '/donors/donor_category2', { 
          token: theToken,
